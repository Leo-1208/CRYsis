...
export const GraphList = () => {
  ...
  return extrapolatedData.map((coin) => {
    ...
    const currentLabels = coin.coin_historical_data.map((currentCoin) => {
      if (currentCoin.date && currentCoin.date.seconds) {
        return convertDate(currentCoin.date.seconds);
      } else {
        console.warn(`Undefined date for coin ID: ${coin.coin_id}`);
        return "Invalid Date"; // or any fallback value or simply skip
      }
    });
